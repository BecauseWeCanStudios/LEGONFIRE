...
def worker(n, files, result, classes, colors, regenerate):
	...
	for file in pbar:
		...
		bpath = os.path.join(dir, name + '.npz')
		mpath = os.path.join(dir, name + '.mask.png')
		if not os.path.exists(bpath) or regenerate:
		...
		result[n].append({ 'image': file, 'mask': mpath, 'boolmask': bpath})
...
parser.add_argument('--regenerate', action='store_true', help='Regenerate mask even if it already exists')
...
for i in range(args.threads):
	threads.append(threading.Thread(target=worker, args=(i, files[i * tn:(i + 1) * tn], result, classes, colors, args.regenerate)))
	threads[-1].start()
